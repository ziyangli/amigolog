...
def execute(self, action, action_input=None):
    ...
    ### NAVIGATION ###
        ...
        if nav_action == "goal_pose_2d" or nav_action == "lookat_point_3d" or nav_action == "prepare_grasp_orientation":
            result = execute_smach_statemachine(self, machine)
            if result == "unreachable" or result == "preempted":
                self.speech.speak("My attempt to reach a location failed")
                # exogenous event that plan failed
            if result == "arrived":
                self.speech.speak("I am at the goal location")
            if result == "goal_not_defined":
                self.speech.speak("I was not able to get a goal pose")
        else:
